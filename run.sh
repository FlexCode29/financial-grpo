...
case "$ROLE" in
  ...
  train)
    ...
    HIP_VISIBLE_DEVICES=$GPU_LIST CUDA_VISIBLE_DEVICES=$GPU_LIST accelerate launch --multi_gpu --num-processes=7 train_grpo.py --model_name "$MODEL_NAME" --use_remote_vllm
    ;;
